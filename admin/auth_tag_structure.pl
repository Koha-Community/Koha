...
my $existingauthtypecode = $input->param('existingauthtypecode')
    || '';    # set when we have to create a new framework (in authtype) by copying an old one (in existingauthtype)
...
my ( $template, $loggedinuser, $cookie ) = get_template_and_user(
    {
        template_name => "admin/auth_tag_structure.tt",
        query         => $input,
        type          => "intranet",
...
if ( $op eq 'add_form' ) {
    ...
    if ($searchfield) {
        $sth = $dbh->prepare(
            "select tagfield,liblibrarian,libopac,repeatable,mandatory,authorised_value from auth_tag_structure where tagfield=? and authtypecode=?"
        );
...
################## ADD_VALIDATE ##################################
    ...
    if ( $input->param('modif') ) {
        ...
        $sth = $dbh->prepare(
            "INSERT INTO auth_tag_structure (tagfield,liblibrarian,libopac,repeatable,mandatory,authorised_value,authtypecode) VALUES (?,?,?,?,?,?,?)"
        );
        $sth->execute(
            $tagfield,
            $liblibrarian,
    ...
    print $input->redirect(
        "/cgi-bin/koha/admin/auth_tag_structure.pl?searchfield=$tagfield&amp;authtypecode=$authtypecode");
    exit;
...
################## DELETE_CONFIRM ##################################
    ...
    $sth = $dbh->prepare(
        "select tagfield,liblibrarian,libopac,repeatable,mandatory,authorised_value from auth_tag_structure where tagfield=?"
    );
...
################## DELETE_CONFIRMED ##################################
    ...
    $dbh->do(
        q|delete from auth_tag_structure where tagfield=? and authtypecode=?|, undef, $searchfield,
        $authtypecode
    );
    $dbh->do(
        q|delete from auth_subfield_structure where tagfield=? and authtypecode=?|, undef, $searchfield,
        $authtypecode
    );
    print $input->redirect(
        "/cgi-bin/koha/admin/auth_tag_structure.pl?searchfield=$searchfield&amp;authtypecode=$authtypecode");
    exit;
...
sub duplicate_auth_framework {
    ...
    my $dbh = C4::Context->dbh;
    my $sth = $dbh->prepare(
        "select tagfield,liblibrarian,libopac,repeatable,mandatory,authorised_value from auth_tag_structure where authtypecode=?"
    );
    ...
    while (
        my (
            ...
            $authorised_value, $thesaurus_category, $seealso,      $hidden
        )
        = $sth->fetchrow
        )
    {
