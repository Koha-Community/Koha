...
if ( $op eq 'edit_form' ) {
    ...
    $profile_list  = get_all_profiles(
        {
            ...
            filters => { template_id => [ $template_id, 0 ], creator => 'Patroncards' }
        }
    );
    ...
    if ($template_id) {    # if a template_id was passed in, this is an update to an existing template
        ...
        if ( $cgi->param('profile_id') && ( $card_template->get_attr('template_id') != $cgi->param('profile_id') ) ) {
            if ( $card_template->get_attr('profile_id') > 0 )
            {              # no need to get the old one if there was no profile associated
                my $old_profile =
                    C4::Patroncards::Profile->retrieve( profile_id => $card_template->get_attr('profile_id') );
