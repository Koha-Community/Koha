...
sub _set_selected {
    ...
    my @select_list =
        ();    # we must make a copy of the referent otherwise we modify the original which causes bad things to happen
    my $selected = 0;
SET_SELECTED:
    foreach my $type (@$source_list) {
        if ( ($selection) && ( $type->{'type'} eq $selection ) )
        {      # even if there is no current selection we must still build the select box
            $selected = 1;
        } else {
            $selected = 0;
        }
    ...
    if ( $layout_xml->{'text'} ) {
        while ( scalar @{ $layout_xml->{'text'} } ) {
            ...
            push @text_fields, (
                ...
                "field_"
                    . $field_number
                    . "_text_alignment" => _set_selected( $field_params->{'text_alignment'}, $alignment_types ),
            );
        }
    }
...
UNUSED_TEXT_FIELDS:
    ...
    foreach my $image ( keys %{ $layout_xml->{'images'} } ) {
        ...
        push @images, (
            ...
            $image
                . "_image_source" => _set_selected(
                $layout_xml->{'images'}->{$image}->{'data_source'}->[0]->{'image_source'}, $image_sources
                ),
            $image
                . "_image_name" =>
                _set_selected( $layout_xml->{'images'}->{$image}->{'data_source'}->[0]->{'image_name'}, $image_names ),
        );
    }
    ...
    foreach my $barcode_param ( keys %{ $layout_xml->{'barcode'}->[0] } ) {
        push @barcode,
            ...
            (
                ...
                ? ( "barcode_"
                        . $barcode_param => _set_selected( $layout_xml->{'barcode'}->[0]->{'type'}, $barcode_types ) )
...
CGI_PARAMS:
    ...
    {    # parse the field values and build a hash of the layout for conversion to xml and storage in the db
    ...
    print $cgi->redirect(
        "manage.pl?card_element=layout" . ( $layout_id == -1 ? "&element_id=$layout_id&op=$op&error=101" : '' ) );
    exit;
