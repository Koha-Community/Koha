...
$sth->do(
    "INSERT INTO `labels_batches_tmp` (label_id, batch_id, item_number) SELECT labelid, batch_id, itemnumber FROM labels;"
) or die "DB ERROR: " . $sth->errstr . "\n";
...
$sth->do("UPDATE `labels_batches_tmp` SET branch_code=(SELECT branchcode FROM branches LIMIT 0,1);")
    or die "DB ERROR: " . $sth->errstr . "\n";
...
while ( my $layout = $sth1->fetchrow_hashref() ) {
    ...
    RECORD:
        ...
        $sth->do(
            "UPDATE `labels_layouts_tmp` SET format_string=\'$format_string\' WHERE layout_id = " . $record->{'id'} )
            or die "DB ERROR: " . $sth->errstr . "\n";
    }
}
...
while ( my $template = $sth3->fetchrow_hashref() ) {
    my $sth4 = C4::Context->dbh->prepare(
        "SELECT profile_id FROM printers_profile_tmp WHERE template_id = " . $template->{'template_id'} );
    $sth4->execute or die "DB ERROR: " . $sth4->errstr . "\n";
    my $profile_id = $sth4->fetchrow_hashref();
    next RECORD if $profile_id->{'profile_id'} eq '';
    $sth->do( "UPDATE `labels_templates_tmp` SET profile_id=\'"
