...
sub logaction{
  my ($usernumber,$modulename, $actionname, $objectnumber, $infos)=@_;
	...
	warn "logactions :$usernumber,$modulename, $actionname, $objectnumber, $infos";
	...
	if ($modulename eq "catalogue"){
		...
		if (@filters){
			foreach my $filter (@filters){
				if ($filter->{name} =~ /user/){
					$filter->{value}=~s/\*/%/g;
					$strsth .= " AND ((borrowers.surname like '".$filter->{value}."') OR (borrowers.cardnumber like '".$filter->{value}."') OR (borrowers.firstname like '".$filter->{value}."')) ";
				}elsif ($filter->{name} =~ /action/){
					$filter->{value}=~s/\*/%/g;
					$strsth .= " AND action_logs.action like ".$dbh->quote($filter->{value});
				}elsif ($filter->{name} =~ /from/){
					$filter->{value}=~s/\*/%/g;
					$strsth .= " AND action_logs.timestamp>".$dbh->quote($filter->{value});
