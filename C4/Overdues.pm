...
sub CalcFine {
    ...
    if ( $issuing_rule->{chargeperiod} && ( $units_minus_grace > 0 ) ) {
        ...
        $charge_periods = defined $issuing_rule->{chargeperiod_charge_at}
            && $issuing_rule->{chargeperiod_charge_at} == 1 ? ceil($charge_periods) : floor($charge_periods);
        ...
        foreach my $recall (@recalls) {
            if (    $recall->item_level
                and $recall->item_id == $item->{itemnumber}
                and $issuing_rule->{recall_overdue_fine} )
            {
                $bib_level_recall = 0;
            ...
            && $overdue->status eq 'UNRETURNED' )
        {
            if ($accountline) {
                Koha::Logger->get->debug("Not a unique accountlines record for issue_id $issue_id")
                    ;    # FIXME Do we really need to log that?
