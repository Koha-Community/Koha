...
use C4::Koha
    qw( getFacets GetVariationsOfISBN GetNormalizedUPC GetNormalizedEAN GetNormalizedISBN getitemtypeimagelocation );
    ...
    _ZOOM_event_loop(
        ...
        sub {
            ...
            for my $j ( $first_record .. $last_record ) {
                my $record = eval {
                    $tmpresults[ $i - 1 ]->record( $j - 1 )->raw();    # 0 indexed
                };
        ...
        sub {
            ...
            if ( $servers[ $i - 1 ] =~ /biblioserver/ ) {
                for my $link_value ( sort { $a cmp $b } keys %$facets_counter ) {
                    ...
                    for my $one_facet (
                        sort { $facets_counter->{$link_value}->{$b} <=> $facets_counter->{$link_value}->{$a} }
                        ...
                        )
                    {
                        ...
                        my $facet_max_length  = C4::Context->preference('FacetLabelTruncationLength')
                            || 20;
                        $facet_label_value = substr( $one_facet, 0, $facet_max_length ) . "..."
...
sub _get_facets_from_records {
    ...
    for ( my $j = 0 ; $j < $jmax ; $j++ ) {
        ...
        if ( !defined $marc_record ) {
            warn "ERROR DECODING RECORD - $@: " . $rs->record($j)->raw();
            next;
        }
...
sub _get_facet_from_result_set {
    ...
    my $facet_dom = XML::LibXML->load_xml( string => ($facet) );
...
sub _detect_truncation {
    my ( $operand, $index ) = @_;
    my (
        @nontruncated, @righttruncated, @lefttruncated, @rightlefttruncated,
        @regexpr
    );
    $operand =~ s/^ //g;
...
## all records not indexed in the onloan register (zebra) and all records with a value of lost equal to 0
        ...
        # OR every member of the group
            ...
            my ( $error, undef, undef, undef, undef, $fixed_limit, undef, undef, undef ) =
                buildQuery( undef, undef, undef, $expanded_lim, undef, undef, $lang );
            $limit      .= " and " if $limit || $query;
            $limit      .= "$fixed_limit";
            $limit_desc .= " $limit";
        }
            ...
            if ( ( $operands[$i] // '' ) ne '' ) {
                ...
                if ($auto_truncation) {
                    unless ( $index =~ /,(st-|phr|ext)/ ) {
                        ...
                        $operand =
                            join( " ", map { ( index( $_, "*" ) > 0 ? "$_" : "$_*" ) } split( /\s+/, $operand ) );
                    }
                }
...
sub _build_initial_query {
    ...
    my $operand =
        ( $params->{indexes_set} ) ? $params->{parsed_operand} : $params->{index_plus} . $params->{parsed_operand};
    ...
    $params->{query_cgi} .= "&q=" . uri_escape_utf8( $params->{original_operand} )
        if ( $params->{original_operand} ne '' );

    #e.g. " and kw,wrdl: test"
    $params->{query_desc} .=
        " " . $operator . " " . ( $params->{index_plus} // q{} ) . " " . ( $params->{original_operand} // q{} );

    $params->{previous_operand} = 1
        unless $params->{previous_operand};    #If there is no previous operand, mark this as one
...
sub searchResults {
    ...
    $search_context->{'interface'} = 'opac'
        if !$search_context->{'interface'} || $search_context->{'interface'} ne 'intranet';
    ...
    for ( my $i = $offset ; $i <= $times - 1 ; $i++ ) {
        ...
        my $fw =
              $scan           ? undef
            : $bibliotag < 10 ? GetFrameworkCode( $marcrecord->field($bibliotag)->data )
        ...
        $oldbiblio->{content_identifier_exists} = 1
            if ( $oldbiblio->{normalized_isbn}
            or $oldbiblio->{normalized_oclc}
        ...
        $oldbiblio->{imageurl} =
            $itemtype ? getitemtypeimagelocation( $search_context->{'interface'}, $itemtype->{imageurl} ) : q{};
        ...
        if (   C4::Context->preference('OpacHiddenItemsHidesRecord')
            && $items_count > 0
...
sub enabled_staff_search_views {
    return (
        ...
        ,    # 1 if the staff search allows the Labeled MARC view
    );
}
