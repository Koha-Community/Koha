...
while ( ( my ($biblionumber) = $sth->fetchrow ) ) {
    ...
    eval { $Koharecord = $biblio->metadata->record };
    if ($@) {
...
if ($browser_tag) {
    ...
    foreach ( keys %browser_result ) {
        $sth->execute(
            length($_), $_, $classification->{$_} ? $classification->{$_} : "classification $_",
            $browser_result{$_}->{value}, $browser_result{$_}->{notendnode} ? 0 : 1
...
sub dewey_french {
    return {
        ...
        "004"     =>
            "Informatique, traitement des données : principes, matériel, architecture, traitements, réseaux, périphériques",
        ...
        "004.25"    =>
            "Analyse et conception des systèmes, architecture des ordinateurs, évaluation des performances d'ordinateurs spécifiques",
        ...
        "004.64"  =>
            "Équipements (à bande de base, à bande large, modems, câbles en fibre optique, contrôleurs de périphériques en général)",
        ...
        "541.24"      =>
            "Structure atomique. Organisation spatiale. Loi de périodicité des éléments (tables périodiques : 546.8)",
        ...
        "541.353" =>
            "Réactions photochimiques dues à des radiations particulières. Rayonnement infrarouge. Lumière visible. Rayonnement ultraviolet",
        ...
        "563"       =>
            "Paléozoologie systématique : protozoaires, parazoaires, coelentérés, échinodermes, phylums apparentés",
        ...
        "629.892"     => "Robots",
        "629.895"     =>
            "Commande informatisée de processus. Système de commande optimale. Système de commande à entrée stochastique",
