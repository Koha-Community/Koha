...
my $mailtext = "<date>;<firstname>;<lastname>;<address1>;<address2>;<postcode>;<city>;<email>;<itemcount>;<titles>\n";
#
# END OF PARAMETERS
#
open OUTFILE, ">Relances.csv" or die "impossible d'ouvrir le fichier de relances";
print OUTFILE "Date;Nom;Prénom;Adresse1;Adresse2;Code_Postal;Ville;Mail;Nblivres;1titre;1auteur;1codebarre;1dateemprunt;1dateret;";
print OUTFILE "2titre;2auteur;2codebarre;2dateemprunt;2dateret;3titre;3auteur;3codebarre;3dateemprunt;3dateret;4titre;4auteur;4codebarre;4dateemprunt;4dateret;5titre;5auteur;5codebarre;5dateemprunt;5dateret;\n";
...
while (($itemcount,$borrnum,$firstname,$lastname,$address1,$address2,$city,$postcode,$email) = $sth->fetchrow) {
                ...
                while (($title, $author, $barcode,$timestamp,$date_due) = $sth2->fetchrow){
                        ...
                        $titles .= ($timestamp?format_date($timestamp):"").";".($date_due?format_date($date_due):"").";" ;
                }
...
#if ($count) {
#                open ODUES, "Relances.csv" or die "Impossible d'ouvrir Relances: $!";
#                my $notice = "$count enregistrements de relances pour fusion\n\n";
#
#                $notice .= <ODUES>;
#                if ($nomail) {
#                        print "TO => $email\n" if $email;
