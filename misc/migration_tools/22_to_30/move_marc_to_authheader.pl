...
while ( my ( $authid, $authtypecode ) = $sth->fetchrow ) {
    ...
    $sth_update->execute(
        $record->as_usmarc(), $record->as_xml("UNIMARCAUTH"),
        $authid
    );
    $totaldone++;
        ...
        if ( $row->{tagorder} ne $prevtagorder || $row->{tag} ne $prevtag ) {
            ...
            if ( $row->{tag} < 10 ) {
                ...
                $field = MARC::Field->new(
                    ( sprintf "%03s", $prevtag ), substr( $row->{tag_indicator} . '  ', 0, 1 ),
                    substr( $row->{tag_indicator} . '  ', 1, 1 ), $row->{'subfieldcode'}, $row->{'subfieldvalue'}
                );
            }
        } else {
            if ( $row->{tag} < 10 ) {
