...
function check_form_borrowers(nav) {
    ...
    if (statut != 1 && document.form.check_member.value > 0) {
        if (!document.form.answernodouble.checked) {
            message_champ += __(
                "Please confirm whether this is a duplicate patron"
            );
            statut = 1;
            document.form.nodouble.value = 0;
        } else {
...
function clone_entry(node) {
    ...
    $("label", clone).attr("for", function () {
        return $(this)
            .attr("for")
            .replace(/patron_attr_\d+/, "patron_attr_" + newId);
    });
...
function update_category_code(category_code) {
    ...
    var min_length = $("select" + category_selector + " option:selected").data(
        "pwdLength"
    );
...
function select_user(borrowernumber, borrower, relationship) {
    ...
    if (is_guarantor) {
        ...
        if (guarantor_id) {
            ...
            fieldset
                .find(".new_guarantor_id_text")
                .first()
                .text(borrower.cardnumber);
            fieldset
                .find(".new_guarantor_link")
                .first()
                .attr(
                    "href",
        ...
        fieldset
            .find(".new_guarantor_surname_text")
            .first()
            .text(guarantor_surname);
        $("#guarantor_surname").val("");

        var guarantor_firstname = $("#guarantor_firstname").val();
        fieldset
            .find(".new_guarantor_firstname")
            .first()
            .val(guarantor_firstname);
        fieldset
            .find(".new_guarantor_firstname_text")
            .first()
            .text(guarantor_firstname);
        ...
        fieldset
            ...
            .first()
            .attr("data-borrowernumber", borrowernumber);
...
function write_age() {
    ...
    if (age.year) {
        age_string +=
            age.year > 1
                ? __("%s years").format(age.year)
                : __("%s year").format(age.year);
        age_string += " ";
    }
    ...
    if (age.month) {
        age_string +=
            age.month > 1
                ? __("%s months").format(age.month)
                : __("%s month").format(age.month);
    }
...
$(document).ready(function () {
    ...
    $(document.body).on("change", ".select_city", function () {
        ...
        $("#" + addressfield + "city,#" + addressfield + "city_quick_add").val(
            matches[2]
        );
        $(
            "#" + addressfield + "state,#" + addressfield + "state_quick_add"
        ).val(matches[3]);
        $(
            "#" +
                addressfield +
    ...
    $("#entryform").validate({
        ...
        submitHandler: function (form) {
            ...
            if (form.beenSubmitted) return false;
    ...
    $("#floating-save").css({
        bottom:
            parseInt($("#floating-save").css("bottom")) +
            $("#changelanguage").height() +
            "px",
    });
    $("#qa-save").css({
        bottom:
            parseInt($("#qa-save").css("bottom")) +
            $("#changelanguage").height() +
            "px",
