...
function validate_goto_page() {
    ...
    if (isNaN(page)) {
        ...
        alert(
            __("The page should be a number between 1 and %s.").format(
                total_pages
            )
        );
        return false;
            ...
            if (tgt.prop("nodeName") == "TD" && !tgt.hasClass("actions")) {
                ...
                tgt.append(
                    $("<div/>", { class: "btn-wrapper" }).append(
                        $("<div/>", { class: "btn-group" }).append(menu_clone)
                    )
                );
            }
        }
...
function InitLastAction() {
    ...
    try {
        last_action = localStorage.getItem("z3950search_last_action");
    } catch (err) {}
    if (last_action) {
        // get short title from attr
        var short_title = $(".z3950actions:eq(0)")
            .siblings(".dropdown-menu")
            .find("a[data-action='" + last_action + "']")
            .attr("title");
...
function ChangeLastAction(action, short_title) {
    ...
    last_action = action;
    if (short_title) {
        // Save choice for preview (MARC or Card)
        $(".z3950actions").text(short_title);
        try {
            localStorage.setItem("z3950search_last_action", last_action);
        } catch (err) {}
    }
}
