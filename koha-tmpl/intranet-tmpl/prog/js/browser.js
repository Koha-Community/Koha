...
KOHA.browser = function (searchid, biblionumber) {
    ...
    if (searches_stored) {
        ...
        if (nb_searches > 20) {
            // No need to keep more than 20 searches
    ...
    var browseRecords = function (movement) {
        ...
        if (newSearchPos > current_search.results.length - 1) {
            ...
            window.location =
                ...
                "&sort_by=" +
                current_search.sort +
                "&gotoPage=detail.pl&gotoNumber=last&searchid=" +
                me.searchid +
                "&offset=" +
                (me.offset - current_search.pagelen);
        } else {
            window.location = window.location.href.replace(
                "biblionumber=" + biblionumber,
                "biblionumber=" + current_search.results[newSearchPos]
            );
        }
    };
    ...
    me.create = function (offset, query, limit, sort, newresults, total) {
        if (current_search) {
            if (
                offset ===
                parseInt(current_search.offset) - newresults.length
            ) {
                current_search.results = newresults.concat(
                ...
                current_search.results =
                    current_search.results.concat(newresults);
            } else {
        ...
        $(document).ready(function () {
            $('#searchresults table tr a[href*="/detail.pl"]').each(
                function () {
                    $(this).attr(
                        "href",
                        $(this).attr("href") + "&searchid=" + me.searchid
                    );
                }
            );
        });
    ...
    me.show = function () {
        if (current_search) {
            ...
            if (
                parseInt(current_search.offset) + me.curPos <=
                current_search.pagelen
            ) {
                // First page
                me.offset = 0;
            } else {
            ...
            $(document).ready(function () {
                if (me.curPos > -1) {
                    ...
                    if (
                        me.curPos === 0 &&
                        parseInt(current_search.offset) === 1
                    ) {
                        prevbutton =
                            '<span id="browse-previous" class="browse-button" title="' +
                            __("Previous") +
                            '"><i class="fa fa-arrow-left"></i></span>';
                    } else {
                        prevbutton =
                            '<a href="#" id="browse-previous" class="browse-button" title="' +
                            __("Previous") +
                            '"><i class="fa fa-arrow-left"></i></a>';
                    }
                    if (
                        parseInt(current_search.offset) + me.curPos ==
                        current_search.total
                    ) {
                        nextbutton =
                            '<span id="browse-next" class="browse-button" title="' +
                            __("Next") +
                            '"><i class="fa fa-arrow-right"></i></span>';
                    } else {
                        nextbutton =
                            '<a href="#" id="browse-next" class="browse-button" title="' +
                            __("Next") +
                    ...
                    $("#menu").before(
                        '<div class="browse-controls"><div class="browse-controls-inner"><div class="browse-label"><a href="' +
                            ...
                            __("Results") +
                            '</a></div><div class="browse-prev-next">' +
                            prevbutton +
                            nextbutton +
                            "</div></div></div>"
                    );
        ...
        if (current_search) {
            $(document).ready(function () {
                ...
                $("#previous_search_link").replaceWith(
                    '<div><div class="browse-label"><a href="' +
                        searchURL +
