...
function check_issues() {
    if (globalfreqdata.unit.length > 0) {
        if (document.f.subtype.value == globalfreqdata.unit) {
            document.f.issuelengthcount.value =
                (document.f.sublength.value * globalfreqdata.issuesperunit) /
                globalfreqdata.unitsperissue;
        } else if (document.f.subtype.value != "issues") {
            alert(
                __(
                    "Frequency and subscription length provided doesn't combine well. Please consider entering an issue count rather than a time period."
                )
            );
        }
    }
...
function Check_page1() {
    ...
    if (bookseller_id.length == 0) {
        input_box = confirm(
            __(
                "If you wish to claim late or missing issues you must link this subscription to a vendor. Click OK to ignore or Cancel to return and enter a vendor"
            )
        );
        if (input_box == false) {
            return false;
...
function Check_page2() {
    ...
    if (
        $("#sublength").val().length == 0 &&
        $("input[name='enddate']").val().length == 0
    ) {
    ...
    if (patternneedtobetested) {
        if (irregularity !== "") {
            alert(
                __(
                    "Warning! Present pattern has planned irregularities. Click on 'Test prediction pattern' to check if it's still valid"
                )
            );
        } else {
            alert(
                __(
                    "Please click on 'Test prediction pattern' before saving subscription."
                )
            );
        }
        return false;
...
function numberpatternload() {
    if ($("#numberpattern option:selected").val() === "mana") {
        ...
        $.getJSON(
            ...
            {
                numberpattern_id: document.f.numbering_pattern.value,
                ajax: "true",
            },
...
function restoreAdvancedPattern() {
    ...
    $("#numberingmethod")
        .prop("readOnly", true)
        .val(globalnumpatterndata.numberingmethod);
...
function testPredictionPattern() {
    ...
    if (more_than_one_serial !== "") {
        ...
        if (nextacquidate == undefined || nextacquidate == "") {
            error_msg +=
                "-" + __("Next issue publication date is not defined") + "\n";
            error++;
        }
    }
    ...
    if (error) {
        alert(
            __(
                "Cannot test prediction pattern for the following reason(s): %s"
            ).format(error_msg)
        );
        return false;
    }
...
function saveAdvancedPattern() {
    ...
    if (found) {
        var msg =
            __("This pattern name already exists. Do you want to modify it?") +
            "\n" +
            __(
                "Warning: This will modify the pattern for all subscriptions that are using it."
            );
        cnfrm = confirm(msg);
    }
        ...
        $.getJSON(
            ...
            function (data) {
                if (data.numberpatternid) {
                    ...
                    alert(
                        __(
                            "Something went wrong. Unable to create a new numbering pattern."
                        )
                    );
                }
            }
...
function mana_search() {
    $("#mana_search").html(
        "<p>" +
            __("Searching for subscription in Mana Knowledge Base") +
            "... <img src='" +
            interface +
            "/" +
            theme +
            "/img/spinner-small.gif' /></p>"
    );
    $("#mana_search").show();
    ...
    $.ajax({
        ...
        if (
            ...
            $("#mana_search").html(
                "<p>" +
                    __("Subscription found on Mana Knowledge Base:") +
                    '</p><p> <a href="#" data-bs-toggle="modal" data-bs-target="#mana_search_result"><i class="fa-solid fa-window-maximize"></i> ' +
                    __("Show Mana results") +
                    "</a></p>"
            );
        } else if ($("#mana_results_datatable").length) {
            $("#mana_search").html(
                "<p>" +
                    __("No subscription found on Mana Knowledge Base") +
                    "</p><p>" +
                    __(
                        "Please feel free to share your pattern with all others librarians once you are done"
                    ) +
                    "</p>"
            );
        } else {
    ...
    $("#biblionumber").on("change", function () {
        ...
        $.ajax({
            ...
            error: function (x) {
                ...
                $("#error_bib_not_exist").html(
                    __("This bibliographic record does not exist")
                );
            },
        });
    });
