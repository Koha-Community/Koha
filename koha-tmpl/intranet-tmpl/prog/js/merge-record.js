...
function build_target_record($sources) {
    ...
    $sources
        .find('.record input[type="checkbox"].fieldpick:checked')
        .each(function () {
            var $checkbox = $(this);
    ...
    $sources
        .find('.record input[type="checkbox"].subfieldpick:checked')
        .each(function () {
            var $checkbox = $(this);
            var $li = $checkbox.parent();
            ...
            for (i in target_field.subfields) {
                ...
                if (code == subfield.code) {
                    alert(
                        __p(
                            "Bibliographic record",
                            "The subfield is non-repeatable and already exists in the destination record. Therefore, you cannot add it."
                        )
                    );
                    return false;
                }
...
function rebuild_target($sources, $target) {
    ...
    $target.empty();
    var keys = $.map(target_record, function (elem, idx) {
        return idx;
    }).sort();
    for (k in keys) {
        var tag = keys[k];
