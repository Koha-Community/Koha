...
function pull_counts() {
    ok_count = parseInt(
        document.getElementById("terms_summary_approved_count").innerHTML
    );
    nok_count = parseInt(
        document.getElementById("terms_summary_unapproved_count").innerHTML
    );
    rej_count = parseInt(
        document.getElementById("terms_summary_rejected_count").innerHTML
    );
}
...
$(document).ready(function () {
    ...
    $("p.check").html(
        '<div id="searchheader"><a id="CheckAll" href="/cgi-bin/koha/tags/review.pl"><i class="fa fa-check" aria-hidden="false"><\/i> ' +
            __("Select all") +
            '<\/a> | <a id="CheckNone" href="/cgi-bin/koha/tags/review.pl"><i class="fa fa-times" aria-hidden="false"><\/i> ' +
            __("Clear all") +
            '<\/a> | <a id="CheckPending" href="/cgi-bin/koha/tags/review.pl"> ' +
    ...
    $("*").ajaxError(function (evt, request, settings) {
        if ((alerted += 1) <= 1) {
            window.alert(__("AJAX error (%s alert)").format(alerted));
        }
    });

    patron_autocomplete($("#approver"), {
