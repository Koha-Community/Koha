...
function openAuth(tagsubfieldid, authtype, source) {
    ...
    var inputs = ul
        ? ul.getElementsByTagName("input")
        : element.parentNode.getElementsByTagName("input");
    for (var myindex = 0; myindex < inputs.length; myindex++) {
        if (inputs[myindex].name && inputs[myindex].name.match(tagsubfield)) {
            ...
            if (
                isNaN(parseInt(subfieldcode)) &&
                inputs[myindex].value != "" &&
                subfieldcode != elementsubfcode
            ) {
    ...
    window.open(
        "../authorities/auth_finder.pl?source=" +
            source +
            "&authtypecode=" +
            authtype +
            "&index=" +
            tagsubfieldid +
            "&value_mainstr=" +
            encodeURIComponent(mainmainstring) +
            "&value_main=" +
            encodeURIComponent(mainstring),
        "_blank",
        "width=800,height=550,toolbar=false,scrollbars=yes"
    );
}
...
function ExpandField() {
    ...
    for (var i = 0, lislen = lis.length; i < lislen; i++) {
        // foreach li
        if (lis[i].hasAttribute("id") == 0) {
            continue;
        } // li element is specific to Select2
        if (lis[i].getAttribute("id").match(/^subfield/)) {
            // if it s a subfield
...
var Select2Utils = {
    ...
    initSelect2: function (selects) {
        if ($.fn.select2) {
            if (
                window.auth_values_creation === undefined ||
                !auth_values_creation
            ) {
                $(selects)
                    .select2()
                        ...
                        $(this)
                            ...
                            .on("select2:select", function (e) {
                                if (e.params.data.newTag) {
                                    ...
                                    $("#avCreate #new_av_category").html(
                                        category
                                    );
                                    $("#avCreate input[name='category']").val(
                                        category
                                    );
                                    $("#avCreate input[name='value']").val("");
                                    $(
                                        "#avCreate input[name='description']"
...
function CloneField(index, hideMarc, advancedMARCEditor) {
    ...
    var divs = Array.from(clone.getElementsByTagName("li")).concat(
        Array.from(clone.getElementsByTagName("div"))
    );
    ...
    for (var i = 0; i < 2; i++) {
        ...
        indicator.setAttribute(
            "name",
            indicator.getAttribute("name") + new_key
        );
    }
    ...
    for (i = 0; i < divslen; i++) {
        ...
        if (
            ...
            divs[i].getAttribute("id").match(/^subfield/)
        ) {
            // if it s a subfield
            ...
            for (j = 0; j < inputs.length; j++) {
                if (
                    inputs[j].getAttribute("id") &&
                    inputs[j].getAttribute("id").match(/^tag_/)
                ) {
                    inputs[j].value = "";

                    //Remove the color added by the automatic linker
                    $(inputs[j]).removeClass(
                        "matching_authority_field no_matching_authority_field"
                    );
                }
            }
            ...
            for (j = 0; j < textareas.length; j++) {
                if (
                    textareas[j].getAttribute("id") &&
                    textareas[j].getAttribute("id").match(/^tag_/)
                ) {
                    textareas[j].value = "";
                }
            }
            ...
            if (inputs.length > 0) {
                inputs[0].setAttribute(
                    "id",
                    inputs[0].getAttribute("id") + new_key
                );
                inputs[0].setAttribute(
                    "name",
                    inputs[0].getAttribute("name") + new_key
                );
                ...
                try {
                    ...
                    inputs[1].setAttribute(
                        "name",
                        inputs[1].getAttribute("name") + new_key
                    );
                } catch (e) {
                    try {
                        ...
                        selects[0].setAttribute(
                            "name",
                            selects[0].getAttribute("name") + new_key
                        );
                        selects[0].selectedIndex = -1;
                    } catch (e2) {
                        // it is a textarea if it s not a select or an input
                        var textareas =
                            divs[i].getElementsByTagName("textarea");
                        if (textareas.length > 0) {
                            id_input =
                                textareas[0].getAttribute("id") + new_key;
                            textareas[0].setAttribute("id", id_input);
                            textareas[0].setAttribute(
                                "name",
                                textareas[0].getAttribute("name") + new_key
                            );
                        }
                    }
                }
            ...
            try {
                ...
                for (var j = 0; j < anchors.length; j++) {
                    if (anchors[j].getAttribute("class") == "buttonPlus") {
                        ...
                        anchors[j].getAttribute("class") == "buttonMinus"
                    ) {
                        anchors[j].setAttribute(
                            "onclick",
                            "UnCloneField('" +
            ...
            if (spans) {
                var buttonDot;
                if (!CloneButtonPlus) {
                    // it s impossible to have  + ... (buttonDot AND buttonPlus)
                    buttonDot = spans[0];
                    if (buttonDot) {
                        // 2 possibilities :
                        try {
            ...
            // it's a indicator div
            if (
                divs[i].getAttribute("id") &&
                divs[i].getAttribute("id").match(/^div_indicator/)
            ) {
                // setting a new id for the indicator div
                divs[i].setAttribute(
                    "id",
                    divs[i].getAttribute("id") + new_key
                );
                ...
                inputs[1].setAttribute(
                    "id",
                    inputs[1].getAttribute("id") + new_key
                );
                ...
                try {
                    ...
                    for (j = 0; j < anchors.length; j++) {
                        if (anchors[j].getAttribute("class") == "buttonPlus") {
                            ...
                            anchors[j].setAttribute(
                                "onclick",
                                "UnCloneField('" + new_id + "'); return false;"
                            );
                        } else if (
                            anchors[j].getAttribute("class") == "expandfield"
                        ) {
...
function CloneSubfield(index, advancedMARCEditor) {
    ...
    for (var i = 0, len = inputs.length; i < len; i++) {
        ...
        inputs[i].setAttribute(
            "name",
            inputs[i].getAttribute("name") + new_key
        );
        if (
            inputs[i].getAttribute("id") &&
            inputs[i].getAttribute("id").match(/^tag_/)
        ) {
            inputs[i].value = "";
        }
        linkid = id_input;
    ...
    for (i = 0, len = selects.length; i < len; i++) {
        ...
        selects[i].setAttribute(
            "name",
            selects[i].getAttribute("name") + new_key
        );
    ...
    for (i = 0, len = textareas.length; i < len; i++) {
        ...
        textareas[i].setAttribute(
            "id",
            textareas[i].getAttribute("id") + new_key
        );
        textareas[i].setAttribute(
            "name",
            textareas[i].getAttribute("name") + new_key
        );
        if (
            textareas[i].getAttribute("id") &&
            textareas[i].getAttribute("id").match(/^tag_/)
        ) {
            textareas[i].value = "";
        }
        linkid = id_input;
...
function CheckMandatorySubfields(p) {
    var total = 0;
    $(p)
        .find(".subfield_line input[name='mandatory'][value='1']")
        .each(function () {
            var editor = $(this).siblings(".input_marceditor");
            if (!editor.length) {
                // Deal with date inputs
                editor = $(this)
                    .siblings(".flatpickr_wrapper")
...
function CheckImportantSubfields(p) {
    var total = 0;
    $(p)
        .find(".subfield_line input[name='important'][value='1']")
        .each(function (i) {
            var editor = $(this).siblings(".input_marceditor");
            if (!editor.length) {
                // Deal with date inputs
...
$(document).ready(function () {
    ...
    if (window.editor === undefined) {
        // TODO This does not work with the advanced editor
                ...
                .then(
                    ...
                    error => {
                        $(".avCreate_error")
                            .html(
                                __(
                                    "Something went wrong. Maybe the value already exists?"
                                )
                            )
                            .show();
                    }
