...
function addItem(node, unique_item_fields) {
    ...
    if (
        $("#items_list table").find('tr[idblock="' + index + '"]').length == 0
    ) {
        if (current_qty < max_qty) {
            ...
            $("#" + index)
                .find("input[name='buttonPlus']")
                .val(__("Update item"));
            $("#" + index)
                .find("input[name='buttonPlusMulti']")
                .remove();
            $("#" + index)
                .find("input[name='multiValue']")
                .remove();
            $("#quantity")
                .val(current_qty + 1)
                .change();
        ...
        if (current_qty < max_qty) cloneItemBlock(index, unique_item_fields);
        var tr = constructTrNode(index);
        $("#items_list table")
            .find('tr[idblock="' + index + '"]:first')
            .replaceWith(tr);
    }
    $("#" + index).hide();
}
    ...
    for (var i = 0; i < count; i++) {
        cloneItemBlock(index, unique_item_fields, function (cloneIndex) {
            addItemInList(cloneIndex, unique_item_fields, function () {
                if (
                    $("#items_list tbody tr").length - countItemsBefore ==
                    count
                ) {
                    $("#multiValue").val("");
                    $("#" + index).appendTo("#outeritemblock");
                    $("#" + index).show();
                    $("#" + index + " #add_multiple_copies").css(
                        "visibility",
                        "hidden"
                    );
    ...
    $("#outeritemblock")
        ...
        .each(function () {
            if ($(this).attr("id") == index) {
                ...
                if (
                    $("#items_list table").find(
                        'tr[idblock="' + $(this).attr("id") + '"]'
                    ).length == 0
                ) {
                    $(this).remove();
                } else {
...
function constructTrNode(index, unique_item_fields) {
    ...
    var del_link =
        ...
        '", "' +
        unique_item_fields +
        "\");' class='btn btn-default btn-xs'><i class='fa fa-trash-can'></i> " +
        __("Delete") +
        "</a>";
...
function deleteItemBlock(node_a, index, unique_item_fields) {
    ...
    if (current_qty - 1 == 0) $("#items_list").hide();
...
function cloneItemBlock(index, unique_item_fields, callback) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            buttonPlus +=
                '<input type="button" class="addItemControl" name="buttonPlus" style="cursor:pointer; margin:0 1em;" onclick="addItem(this,\'' +
                unique_item_fields +
                '\')" value="' +
                __("Add item") +
                '" />';
            buttonPlus +=
                '<input type="button" class="addItemControl cancel" name="buttonClear" style="cursor:pointer;" onclick="clearItemBlock(this)" value="' +
                __("Clear") +
                '" />';
            buttonPlus +=
                '<input type="button" class="addItemControl" name="buttonPlusMulti" onclick="javascript:this.nextSibling.style.display=\'inline\'; this.nextSibling.style.visibility=\'visible\'; return false;" style="cursor:pointer; margin:0 1em;" value="' +
                __("Add multiple items") +
                '" />';
            buttonPlus +=
                '<span id="add_multiple_copies" style="display:none">' +
                '<input type="text" inputmode="numeric" pattern="[0-9]*" class="addItemControl" id="multiValue" name="multiValue" placeholder="' +
                __("Number of items to add") +
                '" />' +
                '<input type="button" class="addItemControl" name="buttonAddMulti" style="cursor:pointer; margin:0 1em;" onclick="checkCount( this ,\'' +
                unique_item_fields +
                '\')" value="' +
                __("Add") +
                '" />' +
            ...
            $(original)
                .find("input[name='field_value']")
                .each(function () {
                    var kohafield = $(this)
                        .siblings("input[name='kohafield']")
                        .val();
                    if (
                        $(this).val() &&
                        $.inArray(kohafield, dont_copy_fields) == -1
                    ) {
                        $(this)
                            .closest("li div")
                            .attr("id")
...
function check_additem(unique_item_fields) {
    ...
    for (var field in array_fields) {
        ...
        $("[name='kohafield'][value='items." + fieldname + "']").each(
            function () {
