...
KOHA.OverDriveCirculation = new (function () {
    ...
    $(document).ready(function () {
        ...
        if (p) {
            try {
                ...
                return;
            } //Catch error if opener is not accessible
            if (cb) {
                cb();
            } else {
                p.location.reload();
    ...
    function display_account(container, data) {
        ...
        var button_toolbar = $("<div/>")
            .addClass("btn-toolbar")
            .attr("role", "toolbar");

        var overdrive_link = $("<div/>")
            .addClass("btn-group mr-2")
            .attr("role", "group")
            .append(
                $(
                    '<a href="https://www.overdrive.com/account/" target="overdrive-account" class="btn btn-sm btn-primary overdrive-link">'
        ...
        var logout_link = $("<div/>")
            ...
            .append(
                $(
                    '<a href="#logout" class="btn btn-sm btn-primary overdrive-logout">'
                )
                    .click(function (e) {
                        e.preventDefault();
        ...
        if (data.checkouts) {
            var checkouts_div = $('<div class="overdrive-div">').html(
                "<h3>" + __("Checkouts") + "</h3>"
            );
        ...
        if (data.holds) {
            var holds_div = $('<div class="overdrive-div">').html(
                "<h3>" + __("Holds") + "</h3>"
            );
    ...
    function item_line(ul_el, item) {
        ...
        var item_details = $('<div class="overdrive-item-details">')
            .append($('<h4 class="overdrive-item-title">').text(item.title))
            .append($('<div class="overdrive-item-author">').text(item.author))
            .append(
                $('<div class="overdrive-item-subtitle">').html(item.subtitle)
            )
            .appendTo(line);
        ...
        svc_ajax("post", { action: "cud-login", password: p }, function (data) {
            ...
            if (data.login_success) {
                ...
                if ($("#overdrive-results-page").length > 0) {
                    ...
                    KOHA.OverDriveCirculation.display_account_details(
                        $("#opac-user-overdrive_panel")
                    );
                }
            }
        });
    ...
    function ajax_button(label, on_click, uniqueName) {
        var button = $('<a href="#">').click(function (e) {
            e.preventDefault();
            on_click();
        });
    ...
    function checkout_format(el, id, formats, copies_available) {
        if (formats.length == 0) {
            alert(
                __("Item cannot be checked out. There are no available formats")
            );
            return false;
        }

        var checkout_format_list = checkout_popup
            .find("#overdrive-format-list")
            .empty();
        formats.forEach(function (item) {
            ...
            var input =
                ...
                item +
                '" name="checkout-format" type="radio" /> ';
        ...
        checkout_popup.find(".overdrive-checkout-submit").click(function (e) {
            e.preventDefault();
            var format = checkout_format_list
                .find("input[type='radio'][name='checkout-format']:checked")
                .val();
            item_action(
                { action: "cud-checkout-format", id: id, format: format },
                el,
                copies_available
            );
            $(this).unbind(e);
