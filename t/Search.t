...
subtest "_build_initial_query tests" => sub {
    ...
    ( $query, $query_cgi, $query_desc, $previous_operand ) = C4::Search::_build_initial_query($params);
    is(
        $query, "query operator parsed_operand",
        "\$query built correctly"
    );
    is(
        $query_cgi, "query_cgi&op=operator&idx=index&q=original_operand",
    ...
    is(
        $query_cgi, "query_cgi&op=AND&idx=index&q=original_operand",
    ...
    is(
        $query_desc, "query_desc  index_plus original_operand",
    ...
    is(
        $query_cgi, "query_cgi&op=operator&q=original_operand",
