...
subtest 'General Add, Get and Del tests' => sub {
    ...
    cmp_ok(
        $item->biblioitemnumber, '==', $biblio->biblioitem->biblioitemnumber,
        "New item is linked to correct biblioitemnumber."
    );

    # Get item.
    my $getitem = Koha::Items->find($itemnumber);
    cmp_ok( $getitem->itemnumber,       '==', $itemnumber,             "Retrieved item has correct itemnumber." );
    cmp_ok( $getitem->biblioitemnumber, '==', $item->biblioitemnumber, "Retrieved item has correct biblioitemnumber." )
        ;    # We are not testing anything useful here
    ...
    $getitem = Koha::Items->find($itemnumber);
    is(
        $getitem->effective_itemtype, $biblio->biblioitem->itemtype,
        "Itemtype set correctly when not using item-level_itypes"
    );
...
subtest 'ModItemTransfer tests' => sub {
    ...
    ModItemTransfer(
        $item->itemnumber,       $library1->{branchcode},
        $library2->{branchcode}, 'Manual'
    );

    my $transfers = Koha::Item::Transfers->search( { itemnumber => $item->itemnumber } );
    ...
    ModItemTransfer(
        $item->itemnumber,       $library2->{branchcode},
...
subtest q{Test Koha::Database->schema()->resultset('Item')->itemtype()} => sub {
    ...
    t::lib::Mocks::mock_preference( 'item-level_itypes', 0 );
    is(
        $item->effective_itemtype(), 'BIB_LEVEL',
        '$item->itemtype() returns biblioitem.itemtype when item-level_itypes is disabled'
    );

    t::lib::Mocks::mock_preference( 'item-level_itypes', 1 );
    is(
        $item->effective_itemtype(), 'ITEM_LEVEL',
        '$item->itemtype() returns items.itype when item-level_itypes is enabled'
    );
...
subtest 'SearchItems test' => sub {
    ...
    $dbh->do(
        'DELETE FROM marc_subfield_structure WHERE tagfield=? AND tagsubfield="z" AND frameworkcode=?', undef,
        $itemfield,                                                                                     $frameworkcode
    );
    $dbh->do(
        'INSERT INTO marc_subfield_structure (tagfield, tagsubfield, frameworkcode) VALUES (?, "z", ?)', undef,
        $itemfield,                                                                                      $frameworkcode
    );
    ...
    my ( undef, undef, $item3_itemnumber ) = AddItemFromMarc(
        $item3_record,
        $biblio->biblionumber
    );
    ...
    # linked to it)
    $dbh->do(
        'DELETE FROM marc_subfield_structure WHERE kohafield="items.itemnotes" AND frameworkcode=?', undef,
        $itemfield,                                                                                  $frameworkcode
    );
    $dbh->do(
        'UPDATE marc_subfield_structure SET kohafield="items.itemnotes" WHERE tagfield=? AND tagsubfield="z" AND frameworkcode=?',
        undef, $itemfield, $frameworkcode
    );
...
subtest 'ModItemFromMarc' => sub {
    ...
    my ( undef, undef, $itemnumber ) = AddItemFromMarc(
        $item_record,
        $biblio->biblionumber
    );
    ...
    subtest 'dateaccessioned' => sub {
        ...
        ModItemFromMarc( $marc, $item->biblionumber, $item->itemnumber );
        is(
            $item->get_from_storage->dateaccessioned, '2022-03-19',
            'dateaccessioned has not been updated if not passed'
        );
...
subtest 'ModDateLastSeen' => sub {
    ...
    t::lib::Mocks::mock_preference( 'CataloguingLog', '1' );
    my $logs_before =
        Koha::ActionLogs->search( { module => 'CATALOGUING', action => 'MODIFY', object => $item->itemnumber } )->count;
