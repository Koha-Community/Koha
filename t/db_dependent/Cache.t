...
SKIP: {
    ...
    $cache->set_in_cache( "clear_me", "I AM MORE DATA", { expiry => 1000 } );    # overly large expiry time, clear below
    ...
    is(
        $cache->get_from_cache('test_deep_copy'), 'just a simple scalar',
        'A scalar will not be modified in the cache if get from the cache'
    );
    ...
    is_deeply(
        $cache->get_from_cache('test_deep_copy_hash'), { a => 'hashref' },
        'A hash will be deep copied when set in cache'
    );
    ...
    %$item_from_cache = ( another => 'hashref' );
    is_deeply(
        $cache->get_from_cache( 'test_deep_copy_hash', { unsafe => 1 } ), { another => 'hashref' },
        'A hash will not be deep copied if the unsafe flag is set'
    );
}
...
subtest 'Koha::Cache::Memory::Lite' => sub {
    ...
    # test fetching an item that isnt in the cache
    is(
        $memory_cache->get_from_cache("not in here"),
        undef, "fetching item NOT in cache"
    );
    ...
    $memory_cache->flush;
    is(
        $memory_cache->get_from_cache("flush_me"),
        undef, "fetching flushed item from cache"
    );
    is(
        $memory_cache->get_from_cache("dont_clear_me"),
