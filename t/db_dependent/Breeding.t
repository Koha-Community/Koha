...
subtest BreedingSearch => sub {
    ...
    my $import_biblio_1 = $builder->build(
        {
            source => 'ImportBiblio',
            value  => {
    ...
    my $import_biblio_2 = $builder->build(
        {
            source => 'ImportBiblio',
            value  => {
...
sub test_build_translate_query {
    ...
    is(
        defined $queries[0] && $queries[0] eq '' && defined $queries[1] && $queries[1] eq '', 1,
        '_bib_build_query gets no parameters'
    );
    ...
    is(
        $queries[0] eq $queries2[0] && $queries[1] eq $queries2[1], 1,
    ...
    is(
        $zquery, '(aut.type any "geo ram_nc ram_ge ram_pe ram_co") and aut.accesspoint="mysubject"',
        'SRU query with double quotes in index'
    );
}
...
sub test_do_xslt {
    ...
    is(
        $res[0]->subfield( '245', 'a' ), 'Just a title',
