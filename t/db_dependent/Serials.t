...
subtest 'test_updateClaim' => sub {
    ...
    $builder->build({
        source => 'Serial',
        value => { serialid => $serialids_1->[0], serialseq => 'serialseq', subscriptionid => $subscriptionid, status => 3, biblionumber => 12345, claimdate => $claimdate_1, claims_count => $claim_count_1 },
    });
    $builder->build({
        source => 'Serial',
        value => { serialid => $serialids_1->[1], serialseq => 'serialseq', subscriptionid => $subscriptionid, status => 3, biblionumber => 12345, claimdate => $claimdate_1, claims_count => $claim_count_1 },
    });
    $builder->build({
        source => 'Serial',
        value => { serialid => 90982, serialseq => 'serialseq', subscriptionid => $subscriptionid, status => 3, biblionumber => 12345, claimdate => $claimdate_1, claims_count => $claim_count_1 },
    });
