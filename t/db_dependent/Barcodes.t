...
subtest 'Test generation of annual barcodes from DB values' => sub {
    ...
    warning_like { $barcodeobj = C4::Barcodes->new('annual'); }
    [qr/No max barcode (.*) found\.  Using initial value\./],
        "(annual) Expected complaint regarding no max barcode found";

    my $barcodevalue = $barcodeobj->value();

    my $item_1 = $builder->build_sample_item( { barcode => $barcodevalue } );
    ...
    my $item_2 = $builder->build_sample_item( { barcode => $barcodevalue } );

    $barcodeobj = C4::Barcodes->new('annual');

    is(
        $barcodevalue, $barcodeobj->db_max(),
        '(annual) db_max should equal the greatest barcode in the db when more than 1 present'
    );
    ok(
        $barcodeobj->value() gt $barcodevalue,
        '(annual) new barcode object should be created with value greater and last value inserted into db'
    );
...
subtest 'Test generation of hbyymmincr barcodes from DB values' => sub {
    ...
    is(
        $barcodevalue, $barcodeobj->db_max(),
        '(hbyymmincr) db_max should equal the greatest barcode in the db when more than 1 present'
    );
    ok(
        $barcodeobj->value() gt $barcodevalue,
        '(hbyymmincr) new barcode object should be created with value greater and last value inserted into db'
    );
...
foreach my $format (@formats) {
    ...
    foreach my $testval ( @{ $thash{$format} } ) {
        if ( $format eq 'hbyymmincr' ) {
            ...
            ok(
                $branch eq $obj2->branch(),
                "$pre branch extended to derived object : " . ( $obj2->branch || 'FAILED' )
            );
