...
subtest 'UseLocationAsAQInSIP syspref tests' => sub {
    ...
    is(
        $sip_item->permanent_location, $branchcode,
        "When UseLocationAsAQInSIP is not set SIP item has permanent_location set to value of homebranch"
    );
    ...
    is(
        $sip_item->permanent_location, "PERMANENT_LOCATION",
        "When UseLocationAsAQInSIP is set SIP item has permanent_location set to value of item permanent_location"
    );
...
subtest "Test patron_status_string" => sub {
    ...
    is(
        substr( $patron_status_string, 9, 1 ), q{ },
        "lost_block_checkout = 0 does not block checkouts with 2 lost checkouts"
    );
    ...
    $patron_status_string = C4::SIP::Sip::MsgType::patron_status_string( $sip_patron, $server );
    is(
        substr( $patron_status_string, 9, 1 ), q{Y},
        "lost_block_checkout = 1 does block checkouts with 2 lost checkouts"
    );
    ...
    is(
        substr( $patron_status_string, 9, 1 ), q{Y},
        "lost_block_checkout = 2 does block checkouts with 2 lost checkouts"
    );
    ...
    is(
        substr( $patron_status_string, 9, 1 ), q{ },
        "lost_block_checkout = 3 does not block checkouts with 2 lost checkouts"
    );
    ...
    is(
        substr( $patron_status_string, 9, 1 ), q{ },
        "lost_block_checkout = 2, lost_block_checkout_value = 2 does not block checkouts with 2 lost checkouts where only 1 has itemlost = 2"
    );
    ...
    is(
        substr( $patron_status_string, 9, 1 ), q{Y},
        "lost_block_checkout = 2, lost_block_checkout_value = 2 does block checkouts with 2 lost checkouts where only 1 has itemlost = 2"
    );
...
subtest "Test build_additional_item_fields_string" => sub {
    ...
    is(
        $attribute_string, sprintf( "XY%s|YZ%s|", $item->itemnumber, $item->biblionumber ),
...
subtest "Test build_custom_field_string" => sub {
    ...
    $attribute_string                                        = $ils_item->build_additional_item_fields_string($server);
    is(
        $attribute_string, sprintf( "ZY%s!|ZX%s*|", $item_id, $item_barcode ),
        'Attribute field generated correctly with multiple params'
    );
    ...
    my $issue = Koha::Checkout->new(
        {
            ...
            itemnumber => $item_object->itemnumber, date_due       => "1999-01-01 12:59:00"
        }
    )->store;
    $siprequest =
          ITEM_INFORMATION
...
sub test_request_patron_info_v2 {
    ...
    check_field(
        $respcode, $response, FID_PATRON_BIRTHDATE, undef,
        'Date of birth successfully stripped from response'
    );
    ...
    check_field( $respcode, $response, FID_HOME_ADDR, undef, 'Home address successfully stripped from response' );
    check_field(
        $respcode, $response, FID_EMAIL, $patron2->{email},
        'Email address successfully retained in response'
    );
    check_field(
        $respcode, $response, FID_HOME_PHONE, $patron2->{phone},
        'Home phone successfully retained in from response'
    );
    check_field(
        $respcode, $response, FID_PATRON_BIRTHDATE, undef,
        'Date of birth successfully stripped from response'
...
sub test_checkout_v2 {
    ...
    my $respcode = substr( $response, 0, 2 );
    check_field(
        $respcode,          $response, FID_SCREEN_MSG, 'This item was previously checked out by you',
        'Check screen msg', 'equals'
    );

    is(
        Koha::Checkouts->search( { itemnumber => $item_object->id } )->count, 0,
        "Item was not checked out (prevcheckout_block_checkout enabled)"
    );
    ...
    is(
        Koha::Checkouts->search( { itemnumber => $item_object->id } )->count, 1,
        "Item was checked out (prevcheckout_block_checkout disabled)"
    );
    ...
    $respcode = substr( $response, 0, 2 );
    check_field(
        $respcode,          $response, FID_SCREEN_MSG, 'Item type cannot be checked out at this checkout location',
        'Check screen msg', 'equals'
    );

    is(
        Koha::Checkouts->search( { itemnumber => $item_object->id } )->count, 0,
        "Item was not checked out (item type matched blocked_item_types)"
    );
...
sub test_checkin_v2 {
    ...
    $msg->handle_checkin($server);
    is(
        substr( $response, 2, 1 ), '1',
        'OK flag is true now with checked_in_ok flag set when checking in an item that was not checked out'
    );
    ...
    check_field(
        $respcode, $response, FID_ALERT_TYPE, '04',
        'Got FID_ALERT_TYPE (CV) field with value 04 ( needs transfer )'
    );
    ...
    $respcode = substr( $response, 0, 2 );
    check_field(
        $respcode, $response, FID_DESTINATION_LOCATION, q{},
        'FID_DESTINATION_LOCATION (CT) field is empty but present'
    );
    $server->{account}->{checked_in_ok}  = 0;
    $server->{account}->{ct_always_send} = 0;
    ...
    $msg->handle_checkin($server);
    is(
        substr( $response, 2, 1 ), '0',
        'OK flag is false when we check in at the wrong branch and we do not allow it'
    );
    is( substr( $response, 5, 1 ), 'Y', 'Alert flag is set' );
    ...
    is(
        Koha::Checkouts->find( $issue->issue_id ), undef,
    ...
    is(
        substr( $response, 2, 1 ), '1',
        'OK flag is true when we check in an item on hold and we allow it but do not capture it'
    );
    is( substr( $response, 5, 1 ),                                            'Y', 'Alert flag is set' );
...
sub test_hold_patron_bcode {
    ...
    my $item = $builder->build_sample_item( { library => $branchcode } );
