...
my $itemnumber1 =
    $builder->build_sample_item( { biblionumber => $biblionumber1, barcode => $barcode1, %item_infos } )->itemnumber;
...
my $itemnumber2 =
    $builder->build_sample_item( { biblionumber => $biblionumber2, barcode => $barcode2, %item_infos } )->itemnumber;
...
EOS
        is(
            $slip->{content}, $empty_slip,
            'No checked out or overdues return an empty slip, it should return undef (FIXME)'
        );
    };
    ...
    subtest 'Daily loans' => sub {
        ...
        Koha::Checkouts->search( { borrowernumber => $borrower->borrowernumber } )
...
EOS
    ...
    subtest 'Hourly loans' => sub {
        ...
        # Test 1: No overdue
        my (
            $date_due_in_time, $date_due_in_time_as_formatted, $date_due_in_late, $date_due_in_late_as_formatted,
            $issue_date,       $slip, $expected_slip
        );
...
EOS
        ...
        is( $slip->{content}, $expected_slip, 'IssueSlip should return a slip with 2 checkouts' )
            or
            diag( Dumper( Koha::Checkouts->search( { borrowernumber => $borrower->{borrowernumber} } )->unblessed ) );
