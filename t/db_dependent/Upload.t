...
subtest 'permanent_directory and temporary_directory' => sub {
    ...
    is(
        Koha::UploadedFile->permanent_directory, $tempdir,
        'Check permanent directory'
    );
    is(
        C4::Context::temporary_directory, $tempdir,
...
subtest 'Add two uploads in category A' => sub {
    ...
    my $rs  = Koha::UploadedFiles->search( { id => [ split ',', $res ] }, { order_by => { -asc => 'filename' } } );
    my $rec = $rs->next;
...
subtest 'Call search_term with[out] private flag' => sub {
    ...
    is(
        Koha::UploadedFiles->search_term(
            {
                term => 'file', include_private => 1,
            }
        )->count,
        4,
        'Returns now four results'
    );
};
    ...
    $builder->build(
        {
            ...
            value  => { category => 'UPLOAD', authorised_value => 'HAVE_AT_LEAST_ONE', lib => 'Hi there' }
        }
    );
...
subtest 'Testing allows_add_by' => sub {
    ...
    is(
        Koha::Uploader->allows_add_by( $patron->{userid} ),
...
subtest 'Testing delete_temporary' => sub {
    ...
    is(
        Koha::UploadedFiles->search( { permanent => 1 } )->count, 3,
