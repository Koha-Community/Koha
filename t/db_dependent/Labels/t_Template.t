...
ok(
    $template = C4::Labels::Template->new( page_width => 8.5, cols => 3 ),
...
foreach my $key ( keys %{$expect_template} ) {
    ok(
        $expect_template->{$key} eq $template->get_attr($key),
...
foreach my $key ( keys %{$new_attr} ) {
    ...
    ok(
        $new_attr->{$key} eq $template->get_attr($key),
...
foreach my $key ( keys %{$expect_conv} ) {
    ok(
        $expect_conv->{$key} eq $conv_template->get_attr($key),
