...
$order =
    Koha::Acquisition::Orders->search( { subscriptionid => $subscription->{subscriptionid}, datereceived => undef } )
    ->next->unblessed;
...
is(
    scalar(@invoices_linked_to_subscriptions), 1,
    'GetInvoices() can identify invoices that are linked to a subscription'
);
