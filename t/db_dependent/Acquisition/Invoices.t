...
BEGIN {
    use_ok(
        'C4::Acquisition',
        qw( NewBasket GetBasket AddInvoice GetInvoice ModReceiveOrder GetInvoiceDetails GetInvoices ModInvoice CloseInvoice ReopenInvoice MergeInvoices DelInvoice )
    );
}
...
my $invoiceid2 = AddInvoice(
    invoicenumber => 'invoice2', booksellerid => $booksellerid, unknown => "unknown",
...
is(
    $invoices[0]->{invoicenumber}, 'invoice1',
    'GetInvoices() to search by publication/copyright year works (bug 8854)'
);
...
is(
    $invoicesummary1->{'invoicenumber'}, $invoice1->{'invoicenumber'},
    'GetInvoice and GetInvoiceDetails retrieve same information'
);
...
my @invoices_linked_to_subscriptions = map { $_->{is_linked_to_subscriptions} ? $_ : () } @invoices;
is_deeply(
