...
get '/biblios' => sub {
    ...
    $c->stash( "koha.embed", { "suggestions" => { children => { "suggester" => {} } } } );
...
subtest 'objects.search helper' => sub {
    ...
    $builder->build_object(
        {
            class => 'Koha::Cities',
            value => { city_name => 'Manuel' }
        }
    );
    $builder->build_object(
        {
            class => 'Koha::Cities',
            value => { city_name => 'Manuela' }
        }
    );
    $builder->build_object(
        {
            class => 'Koha::Cities',
            value => { city_name => 'Manuelab' }
        }
    );
    ...
    $t->get_ok('/cities?_page=1&_per_page=-1')->status_is(200);

    $response_count = scalar @{ $t->tx->res->json };
    is( $response_count, 24, '_per_page=-1 means all resources' );

    $t->get_ok('/cities?_page=100&_per_page=-1')->status_is(200);
    ...
    my $suggestion1 = $builder->build_object(
        {
            ...
            value => { suggestedby => $patron1->borrowernumber, biblionumber => $biblio1->biblionumber }
        }
    );
    my $suggestion2 = $builder->build_object(
        {
            class => "Koha::Suggestions",
            value => { suggestedby => $patron2->borrowernumber, biblionumber => $biblio2->biblionumber }
        }
    );
    my $suggestion3 = $builder->build_object(
        {
            class => "Koha::Suggestions",
            value => { suggestedby => $patron2->borrowernumber, biblionumber => $biblio3->biblionumber }
        }
    );
...
subtest 'objects.search helper order by embedded columns' => sub {
    ...
    my $t = Test::Mojo->new;
    $t->get_ok( '/biblios?_order_by=-suggestions.suggester.firstname' => json =>
...
subtest 'objects.find helper' => sub {
    ...
    $t->get_ok( '/cities/' . $city_1->id )->status_is(200)->json_is( $city_1->to_api );

    $t->get_ok( '/cities/' . $city_2->id )->status_is(200)->json_is( $city_2->to_api );

    # Remove the city
    my $city_2_id = $city_2->id;
    $city_2->delete;
    $t->get_ok( '/cities/' . $city_2_id )->status_is(200)->json_is(undef);
...
subtest 'objects.find helper, embed' => sub {
    ...
    $t->get_ok( '/orders/' . $order->ordernumber )->json_is( $order->to_api( { embed => ( { fund => {} } ) } ) );
...
subtest 'objects.find_rs helper' => sub {
    ...
    $t->get_ok( '/cities/' . $city1->id . '/rs' )->status_is(200)->json_is( '/name' => 'city1' );

    $t->get_ok( '/cities/' . $city2->id . '/rs' )->status_is(200)->json_is( '/name' => 'city2' );

    $t->get_ok( '/cities/' . $city3->id . '/rs' )->status_is(200)->json_is( '/name' => 'city3' );
