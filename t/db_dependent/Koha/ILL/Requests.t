...
subtest 'Basic object tests' => sub {
    ...
    isa_ok(
        $illrq_obj, 'Koha::ILL::Request',
        "Correctly create and load an illrequest object."
    );
    isa_ok(
        $illrq_obj->_config, 'Koha::ILL::Request::Config',
    ...
    is(
        Koha::ILL::Requests->search->count, 0,
...
subtest 'store borrowernumber change also updates holds' => sub {
    ...
    is(
        $hold->borrowernumber, $request->borrowernumber,
    ...
    is(
        $hold->borrowernumber, $other_patron->borrowernumber,
        'after change, changed borrowernumber found in holds'
    );

    is(
        $request->borrowernumber, $other_patron->borrowernumber,
    ...
    is(
        $new_request->borrowernumber, $patron->borrowernumber,
...
subtest 'Working with related objects' => sub {
    ...
    is(
        $illrq->extended_attributes->count,
...
subtest 'Backend testing (mocks)' => sub {
    ...
    is(
        $illrq->custom_capability( 'identity', { test => 1, method => 'blah' } )->{test}, 1,
...
subtest 'Backend core methods' => sub {
    ...
    # expand_template:
    is_deeply(
        $illrq->expand_template( { test => 1, method => "bar" } ),
        {
            test          => 1,
            method        => "bar",
    ...
    is_deeply(
        $illrq->backend_illview( { test => 1 } ), 0,
    ...
    is_deeply(
        $illrq->backend_update_status( { test => 1 } ),
        {
    ...
    my $gen_conf = $illrq->generic_confirm( { current_branchcode => $illbrn->{branchcode} } );
    isnt(
        index( $gen_conf->{value}->{draft}->{body}, $backend->metadata->{Test} ), -1,
        "Generic confirm: draft contains metadata."
    );
    is(
        $gen_conf->{value}->{partners}->next->borrowernumber, $partner1->{borrowernumber},
        "Generic cofnirm: partner 1 is correct."
    );
    is(
        $gen_conf->{value}->{partners}->next->borrowernumber, $partner2->{borrowernumber},
...
subtest 'Helpers' => sub {
    ...
    is(
        $illrq_obj->getPrefix( { brw_cat => "UNKNOWN", branch => "HDE" } ), "TEST",
        "getPrefix: branch"
    );
    $config->set_series(
        'getPrefixes',
    ...
    is(
        $illrq_obj->getPrefix( { branch => "UNKNOWN" } ), "",
        "getPrefix: default"
    );
    ...
    my $return_staff_cancel_fail = $illrq_obj->send_staff_notice('ILL_REQUEST_CANCEL');
    is_deeply(
        $return_staff_cancel_fail,
        { error => 'notice_not_enabled' },
        "Does not send notices that are not enabled"
    );
    my $queue = $schema->resultset('MessageQueue')->search( { letter_code => 'ILL_REQUEST_CANCEL' } );
    ...
    my $return_staff_cancel = $illrq_obj->send_staff_notice('ILL_REQUEST_CANCEL');
    is_deeply(
        $return_staff_cancel,
        { success => 'notice_queued' },
        "Correct return when staff notice created"
    );
    $queue = $schema->resultset('MessageQueue')->search( { letter_code => 'ILL_REQUEST_CANCEL' } );
...
Attribute: Pages=[% illrequestattributes.pages %]
    ...
    is(
        $not->{content},
        "Thepatronforinterlibraryloansrequest"
            . $illrq_obj->id
            . ",withthefollowingdetails,hasrequestedcancellationofthisILLrequest:-author:myauthor-title:mytitleAttribute:Pages=42",
...
subtest 'Censorship' => sub {
    ...
    is_deeply(
        $censor_out, { foo => 'bar', baz => 564, display_reply_date => 1 },
...
subtest 'Checking out' => sub {
    ...
    #
    # No item_type
    my $form_stage_missing_params = $request->check_out( { stage => 'form' } );
    is_deeply( $form_stage_missing_params->{value}->{errors}, { item_type => 1 } );
    ...
    is_deeply( $form_stage_bad_patron->{value}->{errors}, { inhouse => 1 } );
    ...
    is_deeply( $form_stage_two_items->{value}->{errors}, { itemcount => 1 } );
...
subtest 'Checking Limits' => sub {
    ...
    is_deeply(
        $illrq_obj->getLimits( { type => 'branch', value => "CPL" } ),
        { count => 1, method => 'test' },
        "getLimits: by value."
    );
    is_deeply(
        $illrq_obj->getLimits( { type => 'branch' } ),
    ...
    #_limit_counter
    is(
        $illrq_obj->_limit_counter( 'annual', { branchcode => $illrq_obj->branchcode } ),
        1, "_limit_counter: Initial branch annual count."
    );
    is(
        $illrq_obj->_limit_counter( 'active', { branchcode => $illrq_obj->branchcode } ),
        1, "_limit_counter: Initial branch active count."
    );
    is(
        $illrq_obj->_limit_counter( 'annual', { borrowernumber => $illrq_obj->borrowernumber } ),
        1, "_limit_counter: Initial patron annual count."
    );
    is(
        $illrq_obj->_limit_counter( 'active', { borrowernumber => $illrq_obj->borrowernumber } ),
    ...
    is(
        $illrq_obj->check_limits(
            {
                patron      => $illrq_obj->patron,
                librarycode => $illrq_obj->branchcode
            }
        ),
        1,
        "check_limits: no configuration => no limits."
    );
    ...
    is(
        ...
        0,
        "check_limits: branch active limit exceeded."
    );
    $config->set_always(
        'getLimitRules',
    ...
    is(
        ...
        0,
        "check_limits: branch annual limit exceeded."
    );
    $config->set_always(
        'getLimitRules',
    ...
    is(
        ...
        1,
        "check_limits: branch active limit OK."
    );
    $config->set_always(
        'getLimitRules',
    ...
    is(
        ...
        1,
        "check_limits: branch annual limit OK."
    );
    ...
    is(
        ...
        0,
        "check_limits: patron category active limit exceeded."
    );
    $config->set_always(
        'getLimitRules',
    ...
    is(
        ...
        0,
        "check_limits: patron category annual limit exceeded."
    );
    $config->set_always(
        'getLimitRules',
    ...
    is(
        ...
        1,
        "check_limits: patron category active limit OK."
    );
    $config->set_always(
        'getLimitRules',
    ...
    is(
        ...
        1,
        "check_limits: patron category annual limit OK."
    );
    ...
    is(
        ...
        0,
        "check_limits: patron category veto overrides branch OK."
    );
    $config->set_series(
        'getLimitRules',
    ...
    is(
        ...
        0,
        "check_limits: branch veto overrides patron category OK."
    );
...
subtest 'Custom statuses' => sub {
    ...
    my $ill_req = $builder->build_object(
        {
            class => 'Koha::ILL::Requests',
            value => { status_alias => $av->authorised_value }
        }
    );
    isa_ok(
