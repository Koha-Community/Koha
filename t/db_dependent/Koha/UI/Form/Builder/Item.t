    ...
    subtest 'itemtypes' => sub {
        ...
        my $expected = [
            ...
            map { $_->itemtype }
                ...
                sort { uc( $a->translated_description ) cmp uc( $b->translated_description ) } @itemtypes
        ];
        is_deeply(
            $subfield->{marc_value}->{values},
            $expected,
...
subtest 'prefill_with_default_values' => sub {
    ...
    $subfields =
        Koha::UI::Form::Builder::Item->new( { biblionumber => $biblio->biblionumber } )
...
subtest 'subfields_to_prefill' => sub {
    ...
    my $subfields =
        ...
        { biblionumber => $biblio->biblionumber, item => { more_subfields_xml => $more_subfields_xml } } )
        ->edit_form( { subfields_to_prefill => ['é'] } );
    my ($subfield) = grep { $_->{subfield} eq 'é' } @$subfields;
    is( $subfield->{marc_value}->{value}, 'prefill é', 'Not mapped subfield prefilled if needed' );

    $subfields =
        Koha::UI::Form::Builder::Item->new(
        { biblionumber => $biblio->biblionumber, item => { itemnotes => 'prefill z' } } )
        ->edit_form( { subfields_to_prefill => ['z'] } );
...
subtest 'branchcode' => sub {
    ...
    $subfields =
        Koha::UI::Form::Builder::Item->new( { biblionumber => $biblio->biblionumber } )
        ->edit_form( { branchcode => $library->branchcode } );

    ($subfield) = grep { $_->{kohafield} eq 'items.homebranch' } @$subfields;
    is(
        $subfield->{marc_value}->{default}, $library->branchcode,
        'the correct library should be preselected if branchcode is passed'
    );
};
...
subtest 'default_branches_empty' => sub {
    ...
    $subfields =
        Koha::UI::Form::Builder::Item->new( { biblionumber => $biblio->biblionumber } )
        ->edit_form( { default_branches_empty => 1 } );
...
subtest 'kohafields_to_add_datepicker' => sub {
    ...
    foreach my $itemfield_test_case (@itemfield_test_cases) {
        ...
        if ($subfield) {
            is(
                $subfield->{data_type}, $itemfield_test_case->{expected_type},
                "Correct datetype for $itemfield_test_case->{kohafield}"
            );
        } else {
...
subtest 'subfields_to_allow & ignore_not_allowed_subfields' => sub {
    ...
    $subfields = Koha::UI::Form::Builder::Item->new( { biblionumber => $biblio->biblionumber } )->edit_form(
        {
            subfields_to_allow => [
                sprintf( '%s$%s', $tag_cn,    $subtag_cn ),
                sprintf( '%s$%s', $tag_notes, $subtag_notes )
