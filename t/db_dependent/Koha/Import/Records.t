...
is_deeply(
    $retrieved_record_1->unblessed, $record_1,
    'Find a record by import record id should return the correct record'
);
...
my $match_3 = $builder->build_object(
    {
        class => 'Koha::Import::Record::Matches',
        value => {
            score              => 100,
...
is_deeply(
    $matches->next->unblessed, $match_3->unblessed,
    "Match order is score desc, biblionumber desc, so 3 is first"
);

is_deeply(
    $matches->next->unblessed, $match_1->unblessed,
    "Match order is score desc, biblionumber desc, so 1 is second"
);
is_deeply(
    $matches->next->unblessed, $match_2->unblessed,
    "Match order is score desc, biblionumber desc, so 2 is third"
);
    ...
    my $import_record_biblio = $builder->build_object(
        {
            ...
            value => { import_record_id => $import_record->id }
        }
    );
