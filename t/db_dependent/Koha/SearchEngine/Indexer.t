...
SKIP: {
        ...
        is(
            ref $indexer, 'Koha::SearchEngine::Elasticsearch::Indexer',
            'We get the correct class for Elasticsearch biblios'
        );
        ok( $indexer->index_name =~ /biblios$/, "The index is set correctly for biblios" );
        $indexer = Koha::SearchEngine::Indexer->new( { index => $Koha::SearchEngine::AUTHORITIES_INDEX } );
        is(
            ref $indexer, 'Koha::SearchEngine::Elasticsearch::Indexer',
            'We get the correct class for Elasticsearch authorities'
        );
...
SKIP: {
    ...
    for my $engine (@engines) {
        ...
        [ $engine, 'C4::Biblio', $engine, 'C4::Biblio', $engine, 'C4::Biblio' ],
            "index_records is called for $engine when adding a biblio (ModBiblioMarc)";
        ...
        [
            ...
            "index_records is called for $engine when adding an item (Item->store)";
        warnings_are {
        ...
        warnings_are {
            MarkIssueReturned(
                $issue2->{borrowernumber}, $item2->itemnumber, undef, undef,
                { skip_record_index => 1 }
            );
        }
        ...
        [ $engine, 'C4::Circulation' ],
            "index_records is called once for $engine when calling AddReturn if item not issued";
        $issue = $builder->build(
            {
                source => 'Issue',
                value  => { itemnumber => $item->itemnumber }
        ...
        [ $engine, "Koha::Item", $engine, "Koha::Item" ],
            "index_records is called twice for $engine when moving an item to another biblio (Item->move_to_biblio)";
        warnings_are {
            $item4->move_to_biblio( $biblio2, { skip_record_index => 1 } );
        }
        undef,
            "index_records is not called for $engine when moving an item to another biblio (Item->move_to_biblio) if skip_record_index passed";
        ...
        [ $engine, "Koha::Items", $engine, "Koha::Items", $engine, "Koha::Items" ],
            "index_records is called for all from and to biblios for $engine when adopting items (Items->move_to_biblio(Biblio)";
        ...
        [ $engine, "Koha::Item" ],
            "index_records is called for $engine when calling LostItem with 'force_mark_returned'";
        $builder->build(
            {
                source => 'Issue',
                value  => { itemnumber => $item->itemnumber }
        ...
        undef,
            "index_records is not called for $engine when calling LostItem with 'force_mark_returned' if skip_record_index";
        ...
        warnings_are {
            ModItemTransfer(
                $item->itemnumber, $item->homebranch, $item2->homebranch, 'Manual',
                { skip_record_index => 1 }
            );
        }
