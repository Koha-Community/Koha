...
is(
    $retrieved_library_1->branchname, $new_library_1->branchname,
    'Find a library by branchcode should return the correct library'
);
...
my $srstages = Koha::Libraries->find( $new_library_sr->{branchcode} )->stockrotationstages;
...
subtest '->from_email_address' => sub {
    ...
    is(
        $library_1->from_email_address, $library_1->branchemail,
        'If defined, use branches branchemail address'
    );

    $library_1->branchemail(undef)->store();
    is(
        $library_1->from_email_address, 'admin@mylibrary.com',
    ...
    is(
        $library_1->from_email_address, undef,
...
subtest '->inbound_email_address' => sub {
    ...
    is(
        $library_1->inbound_email_address, $library_1->branchreplyto,
        'If defined, use branches replyto address'
    );

    $library_1->branchreplyto(undef)->store();
    is(
        $library_1->inbound_email_address, $library_1->branchemail,
        'Fallback to branches email address when branchreplyto is undefined'
    );

    $library_1->branchemail(undef)->store();
    is(
        $library_1->inbound_email_address, 'reply@mylibrary.com',
...
subtest '->inbound_ill_address' => sub {
    ...
    is(
        $library_1->inbound_ill_address, $library_1->branchillemail,
        'If defined, use library branchillemail address'
    );

    $library_1->branchillemail(undef)->store();
    is(
        $library_1->inbound_ill_address, 'illdefault@mylibrary.com',
    ...
    is(
        $library_1->inbound_ill_address, $library_1->branchreplyto,
        'Fallback to library replyto address when ILLDefaultStaffEmail is undefined'
    );

    $library_1->branchreplyto(undef)->store();
    is(
        $library_1->inbound_ill_address, $library_1->branchemail,
        'Fallback to branches email address when branchreplyto is undefined'
    );

    $library_1->branchemail(undef)->store();
    is(
        $library_1->inbound_ill_address, 'reply@mylibrary.com',
        'Fallback to ReplytoDefault email address when branchreplyto and branchemail are undefined'
    );

    t::lib::Mocks::mock_preference( 'ReplytoDefault', '' );
    is(
        $library_1->inbound_ill_address, 'admin@mylibrary.com',
        'Fallback to KohaAdminEmailAddress email address when branchreplyto, branchemail and ReplytoDefault are undefined'
    );

    t::lib::Mocks::mock_preference( 'KohaAdminEmailAddress', '' );
    is(
        $library_1->inbound_ill_address, undef,
        'Return undef when  email address when branchreplyto, branchemail, ReplytoDefault and KohaAdminEmailAddress are undefined'
    );
...
subtest 'cash_registers' => sub {
    ...
    is(
        ref($registers), 'Koha::Cash::Registers',
    ...
    $register1->delete;
    is(
        $library->cash_registers->next->id, $register2->id,
        'Koha::Library->cash_registers should return the correct cash registers'
    );
...
subtest 'get_hold_libraries and validate_hold_sibling' => sub {
    ...
    $builder->build_object(
        { class => 'Koha::Library::Groups', value => { parent_id => $root2->id, branchcode => $library5->branchcode } }
    );
    ...
    ok(
        $library1->validate_hold_sibling( { branchcode => $library2->branchcode } ),
        'Library 2 is a valid hold sibling'
    );
    ok(
        !$library1->validate_hold_sibling( { branchcode => $library3->branchcode } ),
        'Library 3 is not a valid hold sibling'
    );

    ok(
        $library3->validate_hold_sibling( { branchcode => $library4->branchcode } ),
        'Library 4 is a valid hold sibling'
    );
    ok(
        $library3->validate_hold_sibling( { branchcode => $library5->branchcode } ),
        'Library 5 is a valid hold sibling'
    );
    ok(
        !$library3->validate_hold_sibling( { branchcode => $library2->branchcode } ),
        'Library 2 is not a valid hold sibling'
    );
