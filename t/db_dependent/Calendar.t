...
subtest 'days_forward' => sub {
    ...
    $builder->build_object(
        {
            class => 'Koha::Library::Calendar::WeeklyClosures',
            value => { library_id => $dayweek_branch, weekday => 0, title => q{Sunday}, description => q{Sunday} }
        }
    );
    ...
    for my $mode (qw(Calendar Datedue Dayweek Days)) {
        my $dw_cal = Koha::Library::Calendar->new(
    ...
    my $dw_cal = Koha::Library::Calendar->new( branchcode => $dayweek_branch, days_mode => 'Dayweek' );
    is(
