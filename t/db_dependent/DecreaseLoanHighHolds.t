...
my $patron_hold =
    Koha::Holds->find( { borrowernumber => $patron->borrowernumber, biblionumber => $item->biblionumber } );
...
is(
    $data->{duration}, 1,
    "Should have a duration of 1 because no specific circulation rules so defaults to system preference"
);
...
( undef, $needsconfirmation ) =
    CanBookBeIssued( $patron, $item->barcode, undef, undef, undef, { override_high_holds => 1 } );
