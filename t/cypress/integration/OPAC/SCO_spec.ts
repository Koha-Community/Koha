describe("SCO", () => {
    ...
    function setSyspref(variable: string, value: string) {
        ...
        cy.request("/cgi-bin/koha/mainpage.pl?logout.x=1").then(loginPage => {
            ...
            cy.request({
                ...
                body: [
                    `login_userid=${encodeURIComponent(Cypress.env("apiUsername"))}`,
                    `login_password=${encodeURIComponent(Cypress.env("apiPassword"))}`,
                ...
                if (!csrfForSvc)
                    throw new Error("CSRF token not found on main page");
