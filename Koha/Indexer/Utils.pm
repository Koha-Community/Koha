...
sub _can_ignore_grs1_cfg_line {
    ...
    return 1
        if $line =~ /^\s*$/
        or $line =~ /^#/
        or $line =~ /^(encoding|name|attset|esetname|marc|systag|xpath)/
        or $line =~ /^all/;    # DOM filter automatically indexes all tokens, so
...
sub _append_grs1_def_to_dom_cfg {
    ...
    my $comment = $dom_cfg->createComment( 'record.abs line ' . $grs1_def->{lineno} . ': ' . $grs1_def->{orig_def} );
