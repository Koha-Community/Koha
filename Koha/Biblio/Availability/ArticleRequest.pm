package Koha::Biblio::Availability::ArticleRequest;
...
sub common_biblio_checks {
    ...
    if (!C4::Context->preference('ArticleRequests')) {
        $self->unavailable(Koha::Exceptions::ArticleRequest::NotAllowed->new);
    }
...
sub _item_looper {
    ...
    my $first_item_avail = Koha::Item::Availability::ArticleRequest->new({
        item => $first_item,
        patron => $patron,
    });
    ...
    foreach my $item (@items) {
        ...
        if ($item_availability->available) {
            ...
            if ($item_availability->unavailable == 1 && exists
                $unavails->{'Koha::Exceptions::ArticleRequest::ItemLevelRequestNotAllowed'}){
                push @{$self->{'item_availabilities'}}, $item_availability;
            } else {
...
sub _item_check {
    ...
    my $item_availability = Koha::Item::Availability::ArticleRequest->new({
        item => $item,
        patron => $patron,
