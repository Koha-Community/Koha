...
sub loadDatas{
    ...
    $query .= 'items.location as location, items.dateaccessioned as acquired_year, ';
    $query .= 'items.biblioitemnumber as biblioitemnumber, items.cn_sort as cn_sort, COALESCE(biblioitems.marcxml, deletedbiblioitems.marcxml) as marcxml, items.barcode as barcode, ';
...
sub loadLastAllowedId{
    my $self = shift;
    my $dbh = C4::Context->dbh;
    my $query = "select MAX(datetime) from statistics ";
    $query .= 'inner join items on statistics.itemnumber=items.itemnumber ';
    $query .= "where usercode in ('HENKILO', 'MUUHUOL', 'KOTIPALVEL', 'LAPSI', 'YHTEISO') and other != 'KONVERSIO' and type in ('issue', 'renew')";
    #$query .= " order by datetime";
    my $stmnt = $dbh->prepare($query);
    $stmnt->execute() or die($DBI::errstr);
...
sub getRowIdAtLimit{
    ...
    $query .= "order by datetime ";
    $query .= "LIMIT 1 OFFSET $limitStart";
    my $stmnt = $dbh->prepare($query);
    if(@parameters){
       $stmnt->execute(@parameters) or die($DBI::errstr);
    }
...
sub getLimitRowCount{
    ...
    my $rowCount = 0;
    my $dbh = C4::Context->dbh;
    my $query = "select count(1) as row_count from statistics ";
    $query .= 'inner join items on statistics.itemnumber=items.itemnumber ';
    $query .= "where datetime = '$idAtLimit' and usercode in ('HENKILO', 'MUUHUOL', 'KOTIPALVEL', 'LAPSI', 'YHTEISO') and other != 'KONVERSIO' and type in ('issue', 'renew')";
    my $stmnt = $dbh->prepare($query);
    $stmnt->execute(@parameters) or die($DBI::errstr);
    if($stmnt->rows == 1){
...
sub getLimitOffset{
    ...
    my $query = "select datetime from statistics ";
    $query .= 'inner join items on statistics.itemnumber=items.itemnumber ';
    $query .= "where datetime > ? and usercode in ('HENKILO', 'MUUHUOL', 'KOTIPALVEL', 'LAPSI', 'YHTEISO') and other != 'KONVERSIO' and type in ('issue', 'renew')";
    $query .= "order by datetime ";
    $query .= "LIMIT $idRowCount OFFSET $limitStart";
    my $stmnt = $dbh->prepare($query);
    $stmnt->execute(@parameters) or die($DBI::errstr);
    if($stmnt->rows > 0){
