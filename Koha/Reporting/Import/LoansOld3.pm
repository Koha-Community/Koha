...
sub loadLastAllowedId{
    my $self = shift;
    my $dbh = C4::Context->dbh;
    my $query = "select MAX(datetime) from statistics where usercode in ('HENKILO', 'MUUHUOL', 'KOTIPALVEL', 'LAPSI', 'YHTEISO') and other != 'KONVERSIO' and type in ('issue', 'renew') order by datetime";
    my $stmnt = $dbh->prepare($query);
    $stmnt->execute() or die($DBI::errstr);
...
sub getLimitRowCount{
    ...
    my $rowCount = 0;
    my $dbh = C4::Context->dbh;
    my $query = "select count(1) as row_count from statistics where datetime = '$idAtLimit' and usercode in ('HENKILO', 'MUUHUOL', 'KOTIPALVEL', 'LAPSI', 'YHTEISO') and other != 'KONVERSIO' and type in ('issue', 'renew')";
    my $stmnt = $dbh->prepare($query);
    $stmnt->execute(@parameters) or die($DBI::errstr);
    if($stmnt->rows == 1){
...
sub getLimitOffset{
    ...
    my $query = "select datetime from statistics where datetime > ? and usercode in ('HENKILO', 'MUUHUOL', 'KOTIPALVEL', 'LAPSI', 'YHTEISO') and other != 'KONVERSIO' and type in ('issue', 'renew')";
