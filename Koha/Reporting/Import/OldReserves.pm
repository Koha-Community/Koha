...
sub loadDatas{
    ...
    my $query .= 'select reserves.reservedate as reservedate, reserves.timestamp as datetime, reserves.reserve_id as reserve_id, "1" as amount, reserves.cancellationdate, reserves.suspend, reserves.found, reserves.pickupexpired, "1" as is_old, ';
...
sub loadLastAllowedId{
    ...
    my $query = "SELECT MAX(timestamp) as datetime from old_reserves";
    my $stmnt = $dbh->prepare($query);
    $stmnt->execute() or die($DBI::errstr);
