...
sub monthly_report {
    ...
    return try {
        ...
        my $usage_data_providers =
          Koha::ERM::UsageDataProviders->search( {}, {} )->unblessed;
        my $metric_types =
...
# Objects with no data in the selected range will not be returned by the API - we still want to include them if they have been requested
        my $requested_ids =
          _get_correct_query_param( $data_type, \@query_params_array,
            'monthly' );
        ...
        for my $data_object ( @{$data} ) {
            ...
            my @provider_object        = grep {
                $usage_data_provider_id eq $_->{erm_usage_data_provider_id}
...
# Titles with no data in the selected range will not be returned by the API - we still want to include them if they have been requested
        ...
        my $metric_types =
          $query_params_array[0]->{'erm_usage_yuses.metric_type'};
        my @report_data;
...
sub _get_missing_data {
    ...
    if ( $data_type eq 'title' ) {
        $item =
          Koha::ERM::UsageTitles->find( { title_id => $id }, {} )->unblessed;
    }
    if ( $data_type eq 'platform' ) {
        $item = Koha::ERM::UsagePlatforms->find( { platform_id => $id }, {} )
          ->unblessed;
    }
    if ( $data_type eq 'database' ) {
        $item = Koha::ERM::UsageDatabases->find( { database_id => $id }, {} )
          ->unblessed;
    }
    if ( $data_type eq 'item' ) {
        $item =
          Koha::ERM::UsageItems->find( { item_id => $id }, {} )->unblessed;
    }
