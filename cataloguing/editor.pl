...
$template->{VARS}->{editable_batches} = [
    $schema->resultset('ImportBatch')->search(
        {
...
$template->{VARS}->{DefaultLanguageField008} =
    pack( 'A3', C4::Context->preference('DefaultLanguageField008') || 'eng' );
