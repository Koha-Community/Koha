...
if ( $record && $op ne 'cud-do_search' ) {
    $authorcorp       ||= $record->subfield( $authfields_mapping->{'authorcorp'},       'a' );
    $authorpersonal   ||= $record->subfield( $authfields_mapping->{'authorpersonal'},   'a' );
    $authormeetingcon ||= $record->subfield( $authfields_mapping->{'authormeetingcon'}, 'a' );
    $uniformtitle     ||= $record->subfield( $authfields_mapping->{'uniformtitle'},     'a' );
