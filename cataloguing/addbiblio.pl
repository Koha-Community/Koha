            ...
            if ( $record ne -1 && ( $record->field($tag) || $tag eq '000' ) ) {
                ...
                foreach my $field (@fields) {
                    ...
                    foreach my $subfield ( sort( keys %{ $tagslib->{$tag} } ) ) {
                        ...
                        next
                            ...
                            and $tagslib->{$tag}->{'a'}->{hidden} < 5 );    #check for visibility flag
                    ...
                    )
                {
                    next
                        ...
                        and not( $subfield->{subfield} eq "9"
                        and exists( $tagslib->{$tag}->{'a'}->{authtypecode} )
                        and defined( $tagslib->{$tag}->{'a'}->{authtypecode} )
                        and $tagslib->{$tag}->{'a'}->{authtypecode} ne ""
                        and $tagslib->{$tag}->{'a'}->{hidden} > -4
                ...
                if ( $#subfields_data >= 0 ) {
                    my %tag_data = (
                        ...
                        mandatory  => $tagslib->{$tag}->{mandatory},
                        important  => $tagslib->{$tag}->{important},
                        indicator1 => ( $indicator1 || $tagslib->{$tag}->{ind1_defaultvalue} )
                        ,    #if not set, try to load the default value
                        indicator2 => ( $indicator2 || $tagslib->{$tag}->{ind2_defaultvalue} )
    ...
    && !$breedingid )
{
    ...
    print $input->redirect( '/cgi-bin/koha/cataloguing/editor.pl'
            . ( $biblionumber ? ( ( $op eq 'duplicate' ? '#duplicate/' : '#catalog/' ) . $biblionumber ) : '' ) );
    exit;
}
...
if ($biblionumber) {
    ...
    ( $biblionumbertagfield,  $biblionumbertagsubfield )  = &GetMarcFromKohaField("biblio.biblionumber");
    ( $biblioitemnumtagfield, $biblioitemnumtagsubfield ) = &GetMarcFromKohaField("biblioitems.biblioitemnumber");
