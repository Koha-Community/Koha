...
my $modify         = $input->param('cud-modify');
...
$nodouble = 1 if ($op eq 'cud-modify' or $op eq 'duplicate');    # FIXME hack to represent fact that if we're
...
$template->param( "cud-add" => 1 ) if ( $op eq 'cud-add' );
$template->param( "quickadd" => 1 ) if ( $quickadd );
$template->param( "duplicate" => 1 ) if ( $op eq 'duplicate' );
$template->param( "checked" => 1 ) if ( defined($nodouble) && $nodouble eq 1 );
if ( $op eq 'cud-modify' or $op eq 'cud-save' or $op eq 'duplicate' ) {
...
if ( $op eq 'cud-insert' || $op eq 'cud-modify' || $op eq 'cud-save' || $op eq 'duplicate' ) {
...
if ( ($op eq 'cud-modify' || $op eq 'cud-insert' || $op eq 'cud-save'|| $op eq 'duplicate') and ($step == 0 or $step == 3 )){
...
if ($nok or !$nodouble){
    ...
    $op="cud-modify" if ($op eq "cud-save");
    %data=%newdata; 
...
if (defined ($data{'branchcode'}) and ( $op eq 'cud-modify' || $op eq 'duplicate' || ( $op eq 'cud-add' && $category->category_type eq 'C' ) )) {
