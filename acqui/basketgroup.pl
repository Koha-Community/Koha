...
use C4::Acquisition
    qw( GetOrders GetOrder get_rounded_price GetBasket GetBasketgroup GetBasketsByBasketgroup GetBasketgroups GetBasketsByBookseller ModBasket CloseBasketgroup GetBasketGroupAsCSV DelBasketgroup ReOpenBasketgroup ModBasketgroup NewBasket NewBasketgroup );
use Koha::Database;
...
sub printbasketgrouppdf {
    ...
    my @valid_pdfformats =
        qw(pdfformat::layout3pages pdfformat::layout2pages pdfformat::layout3pagesfr pdfformat::layout2pagesde pdfformat::layout1page);
    ...
    for my $basket (@$baskets) {
        ...
        for my $ord (@ords) {
            ...
            my $itemtype =
                ( $ord->{itemtype} and $biblioitem->itemtype )
    ...
    my $pdf = printpdf(
        $basketgroup, $bookseller, $baskets, \%orders,
        $bookseller->tax_rate // C4::Context->preference("TaxRates")
...
sub generate_edifact_orders {
    ...
    if ($ean) {
        ...
        print $input->redirect(
            '/cgi-bin/koha/acqui/basketgroup.pl?booksellerid=' . $booksellerid . '&message=No%20EDIFACT%20Setup' );
    }
    return;
}
...
my $rs     = $schema->resultset('VendorEdiAccount')->search( { vendor_id => $booksellerid, } );
...
if ( $op eq "add_form" ) {
    ...
    my $redirectpath =
        ( ( defined $input->param('mode') ) && ( $input->param('mode') eq 'singlebg' ) )
        ? '/cgi-bin/koha/acqui/basketgroup.pl?op=add&amp;basketgroupid='
        . $basketgroupid
        . '&amp;booksellerid='
        . $booksellerid
        : '/cgi-bin/koha/acqui/basketgroup.pl?booksellerid=' . $booksellerid . '&amp;listclosed=1';
    ...
    my $redirectpath =
        ...
        . '&amp;booksellerid='
        . $booksellerid
        : '/cgi-bin/koha/acqui/basketgroup.pl?booksellerid=' . $booksellerid;
...
$template->param(
    listclosed => ( ( defined $input->param('listclosed') ) && ( $input->param('listclosed') eq '1' ) ) ? 1 : 0 );
